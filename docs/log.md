# Work log — what happened, and why it was done that way

**This is the history; `CLAUDE.md` is the memory.** Newest first, one entry per
piece of work. Nothing here is loaded into a session automatically, which is the
point: it is 5,000 lines and reading it every morning is what made `CLAUDE.md`
unreadable.

**What lands here.** A bug that is fixed and cannot recur. The evidence behind a
change — which suites ran, what the counts were, what was proved by reverting. The
three wrong diagnoses before the right one. Anything a future session would only
want when asking *"why did we do it that way?"* rather than *"what do I do now?"*.

**What does not.** A rule that still constrains today's work, a trap that can still
be walked into, or the state of a thing as it stands — those are `CLAUDE.md`, and
duplicating them here means two copies that will disagree. A *procedure* is a skill
in `.claude/skills/`. A classroom run is `docs/feedback.md`.

**The routing question, asked at every commit** (`tools/skill-check.js` and
`tools/memory-check.js` between them make it unavoidable):

| What was learned | Where it goes |
|---|---|
| The checklist would have caught it | nowhere — change nothing |
| A general procedure lesson | the skill that covers it |
| A fact about the project's **state** | `CLAUDE.md` |
| **What happened, and the evidence** | **here** |
| What a real class did | `docs/feedback.md` |

Entries below were the `## Current status` section of `CLAUDE.md` until 2026-08-16,
where they had accumulated to 186 entries and 5,000 lines — three quarters of the
file — going back to the first DCU reskin. They are kept verbatim.
...
## Appendix — trimmed from the reference sections, 2026-08-16

The split above moved `## Current status` here whole. The same pass also cut
narrative out of the *reference* sections of `CLAUDE.md` — the backstory of how a
rule came to be written, where the rule itself stayed. Those lines are below,
verbatim and out of order, so nothing is only in git.
...
## Appendix — trimmed from the Next list, 2026-08-16

The Next list was 192 lines, most of it items already struck through as done and
their reasoning. What was cut:
