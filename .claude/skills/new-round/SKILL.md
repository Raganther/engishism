...
**Declare it through `Kit.round.skill(id, spec)`, never by copying a round**: the factory
owns the two faces, the rest as the held answer (the host's Check judges `s.chosen`, so a
rest is written there beside `picks` and `round:arranged` re-reads the button), the
read/judge/accept shape around your `key` and `result`, the lanes and the bare-phone arm.
You declare `key`, `state`, `result`, `table` (`deal`/`rest`/`opts`), `arm` extras, `cue`,
`tell`/`said` — and `measure` when the whole table is the answer (the stack). Its header
in `hub-rounds.js` is the field list; `rounds/line.js`, `plinko.js` and `stack.js` are the
three callers, each under seventy lines. Jeopardy pays a tile and passes a turn when the round says a
