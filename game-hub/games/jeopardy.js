...
(function(){
  ...
  /* ---- Jeopardy's own settings ----
     The shared steal (stealOnWrong/stealFullValue), keepControl and phonePrompt stay in
     the engine; these are Jeopardy's group. */
  ...
  /* ---- the classic rules preset ----
     Each of the three switches is separately useful; `jRules` sets all three at once,
     because "play it like the show" is one decision. Choosing a preset *writes* the
     switches rather than shadowing them, so the rows always say what will happen. What
     the phones do is part of the mode, not a separate decision. */
  const J_PRESETS = {
